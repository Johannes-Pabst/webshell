...
class FileSystem {
    ...
    getEntry(path) {
        ...
        if(path.startsWith("/")){
            ...
            if(this.root.children.home&&this.root.children.home.children.user){
                current = this.root.children.home.children.user;
        ...
        if(parts.length==1&&parts[0]==""&&current!=this.currentDir){
            return current;
        }
    ...
    mkdir(name) {
        ...
        if (name.contains("/")) return 'no "/" allowed!';
    ...
    rm(name, recursive = false) {
        ...
        let target = this.getEntry(name);
        if (!target) return 'No such file or directory';
        if (target.type === 'dir') {
 ...
 `;
    ...
    touch(name) {
        ...
        let target = this.getEntry(name);
        if (target) return 'File already exists';
        target = {
            name,
            type: 'file',
            content: ''
    ...
    nano(name) {
        ...
        nanoEditor.value = target.content;
    ...
    saveNano() {
        ...
        let target = this.getEntry(name);
        if (name && target) {
            target.content = nanoEditor.value;
        }
    ...
    cat(name) {
        ...
        let target = this.getEntry(name);
        if (!target || target.type !== 'file') return 'File not found';
